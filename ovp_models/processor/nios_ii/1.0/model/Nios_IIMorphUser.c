...
static void storeRegister(Nios_IIMorphStateP state, Uns32 size) {
    ...
    // Uns32 address = state->Nios_II->gpr[A] + (Uns32)SIMM16;
    // Uns32 data = state->Nios_II->gpr[B];
    // state->Nios_II->fprint_write_out_address address;
    // state->Nios_II->fprint_write_out_data = data;
    // state->Nios_II->fprint_write_out_address = 1;
...
static void vmic_BankSwitch(Nios_IIP Nios_II, Uns32 old_status, Uns32 new_status) {
    ...
    //
    Uns32 regSets  = Nios_II->params.NUMBER_SHADOW_REGISTER_SETS;
    Uns32 fromSet  = status1.bits.CRS;
    Uns32 toSet    = status2.bits.CRS;
    Bool  validSet = True;
    if (toSet > regSets) {
    ...
    Int32 i;
    for(i=0; i<32; i++) {
        Nios_II->shadow[fromSet][i] = Nios_II->gpr[i];
        Nios_II->gpr[i] = validSet ? Nios_II->shadow[toSet][i] : 0xdeadbeef;
    }
