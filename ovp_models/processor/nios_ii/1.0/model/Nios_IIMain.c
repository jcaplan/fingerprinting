...
VMI_CONSTRUCTOR_FN(Nios_IIConstructor) {
    ...
    Nios_II->iendian = params->endianset ? params->endian : MEM_ENDIAN_LITTLE;
    Nios_II->dendian = params->endianset ? params->endian : MEM_ENDIAN_LITTLE;
    Nios_II->exception = EXC_NONE;

#define COPY_PARAM(_name)  \
    Nios_II->params._name        = params->_name; \
    Nios_II->params._name ## set = params->_name ## set;
