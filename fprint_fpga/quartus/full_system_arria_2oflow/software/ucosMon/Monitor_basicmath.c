...
void print_status_task(void* pdata) {
	while (1) {
		...
		printf("core 0 result %d %f %f %f \ncore 1 result %d %f %f %f",
				answer[0].num_solution, answer[0].x[0], answer[0].x[1],
				answer[0].x[2], answer[1].num_solution, answer[1].x[0],
				answer[1].x[1], answer[1].x[2]);
	}
}
