...
/**********************************************************************
 ...
 setup_brightness_lut() the LUT is setup. This allocas enough space
 for *bp and then repositions the pointer to the centre of the
 allocaed space. The SUSAN function e^-(x^6) or e^-(x^2) is
...
/* ********** Leave the rest - but you may need to remove one or both of sys/file.h and alloca.h lines */
...
//#include <alloca.h>      /* may want to remove this line */
typedef struct {
...
susan_principle(in, r, bp, max_no, x_size, y_size)
	uchar *in, *bp;int *r, max_no, x_size, y_size; {
	int i, j, n;
	uchar *p, *cp;
...
enlarge(in, tmp_image, x_size, y_size, border)
	uchar **in;uchar *tmp_image;int *x_size, *y_size, border; {
	int i, j;
	...
	for (i = 0; i < border; i++) /* copy top and bottom rows; invert as many as necessary */
	{
		memcpy(tmp_image + (border - 1 - i) * (*x_size + 2 * border) + border,
				*in + i * *x_size, *x_size);
		memcpy(
				tmp_image + (*y_size + border + i) * (*x_size + 2 * border)
						+ border, *in + (*y_size - i - 1) * *x_size, *x_size);
	}
...
void susan_smoothing(three_by_three, in, dt, x_size, y_size, bp)
	int three_by_three, x_size, y_size;uchar *in, *bp;float dt; {
	/* {{{ vars */
	...
	if ((2 * mask_size + 1 > x_size) || (2 * mask_size + 1 > y_size)) {
		printf(
				"Mask size (1.5*distance_thresh+1=%d) too big for image (%dx%d).\n",
				mask_size, x_size, y_size);
		exit(0);
	}

	tmp_image = (uchar *) alloca(
	...
	if (three_by_three == 0) { /* large Gaussian masks */
		...
		dp = (unsigned char *) alloca(n_max * n_max);
		dpt = dp;
		temp = -(dt * dt);
...
susan_corners(in, r, bp, max_no, corner_list, x_size, y_size)
	uchar *in, *bp;int *r, max_no, x_size, y_size;CORNER_LIST corner_list; {
	...
	cgx = (int *) alloca(x_size * y_size * sizeof(int));
	cgy = (int *) alloca(x_size * y_size * sizeof(int));
...
void susan(uchar *input,int x_size, int y_size) {
    ...
    int bt = 20;
	uchar bp [516];
	unsigned char in[1824];
	memcpy(in,input,x_size * y_size);
