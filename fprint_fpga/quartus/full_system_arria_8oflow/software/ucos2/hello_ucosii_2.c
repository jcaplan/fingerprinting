...
int *isr_2_oflow_ptr = (int *) PROCESSOR2_0_CPU_OFLOW_IRQ_0_BASE; //Pointer to oflow IRQ Register
int overflow;
								//(CPU0 -> CPU1)
...
static void handle_cpu2_oflow_interrupt(void *context) {
	altera_avalon_mutex_lock(mutex, 1);
	{

		overflow = *isr_2_oflow_ptr & 0x100;
		*isr_2_oflow_ptr = 0;
...
static void init_cpu2_oflow_isr(void) {
	alt_ic_isr_register(PROCESSOR2_0_CPU_OFLOW_IRQ_0_IRQ_INTERRUPT_CONTROLLER_ID,
			PROCESSOR2_0_CPU_OFLOW_IRQ_0_IRQ, handle_cpu2_oflow_interrupt, (void*) NULL,
			(void*) NULL);
}
