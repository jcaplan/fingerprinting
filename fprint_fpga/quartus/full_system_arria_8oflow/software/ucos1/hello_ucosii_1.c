...
int *isr_1_oflow_ptr = (int *) PROCESSOR1_0_CPU_OFLOW_IRQ_0_BASE; //Pointer to oflow IRQ Register
int overflow;
								//(CPU0 -> CPU1)
...
static void handle_cpu1_oflow_interrupt(void *context) {
	altera_avalon_mutex_lock(mutex, 1);
	{

		overflow = *isr_1_oflow_ptr & 0x100;
		if(overflow){
...
static void init_cpu1_oflow_isr(void) {
	alt_ic_isr_register(PROCESSOR1_0_CPU_OFLOW_IRQ_0_IRQ_INTERRUPT_CONTROLLER_ID,
			PROCESSOR1_0_CPU_OFLOW_IRQ_0_IRQ, handle_cpu1_oflow_interrupt, (void*) NULL,
			(void*) NULL);
}
...
//				PREEMPT_TASK_PRIORITY, PREEMPT_TASK_PRIORITY, preempt_task_stk, TASK_STACKSIZE, NULL,0);
	int arg_2 = PREEMPT_TASK_PRIORITY_2;
		OSTaskCreateExt(preemption_task, (void*) &arg_2, (void *) &preempt_task_stk_2[TASK_STACKSIZE - 1],
				2, PREEMPT_TASK_PRIORITY_2, preempt_task_stk_2, TASK_STACKSIZE, NULL,0);
