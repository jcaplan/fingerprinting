...
void set_task_directory(Directory_Init_Struct* d){
	...
	uint32_t* start = (uint32_t*)(COMPARATOR_BASE_ADDRESS + CRC_DIRECTORY_START_BASE);
	*start = (d->core_id << 24) + (d->key<<16) + d->start_ptr;

	//Place the end pointer for the given task
	uint32_t* end   = (uint32_t*)(COMPARATOR_BASE_ADDRESS + CRC_DIRECTORY_END_BASE);
	*end = (d->core_id << 24) + (d->key<<16) + d->end_ptr;
...
void enable_fprint_task(int key){
	...
	uint32_t* fprint_currentstate = (uint32_t*)(FPRINT_BASE_ADDRESS + FPRINT_SPR_CURRENTSTATE);
...
void fprint_reset_irq(void){
	...
	uint32_t* fprint_collision = (uint32_t*)(COMPARATOR_BASE_ADDRESS + COMPARATOR_EXCEPTION_OFFSET);
	*fprint_collision = 0;
}

int fprint_status(Fprint_Status* fps){
		Fprint_Status* f = (uint32_t*)(COMPARATOR_BASE_ADDRESS + COMPARATOR_EXCEPTION_OFFSET);
...
void set_core_assignment(int table_column, int core_id, int task_id){
	uint32_t* fprint_core_entry = (uint32_t*)(COMPARATOR_BASE_ADDRESS + COMPARATOR_CORE_ASSIGNMENT_OFFSET);

    *fprint_core_entry = (table_column << 24) + (task_id << 16) +  core_id;
}
...
void set_nmr(int task_id, int nmr) {
	...
	uint32_t* nmr_reg = (uint32_t*)(COMPARATOR_BASE_ADDRESS + COMPARATOR_NMR_REG_OFFSET);

	*nmr_reg = (task_id << 16) + nmr;
}
