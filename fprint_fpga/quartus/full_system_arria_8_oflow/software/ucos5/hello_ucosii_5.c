...
void (*ct)(int);
void (*pt)(void*);
int *isr_5_ptr = (int *) PROCESSOR5_0_CPU_IRQ_0_BASE;	//Pointer to IRQ Register
int *isr_5_oflow_ptr = (int *) PROCESSOR5_0_CPU_OFLOW_IRQ_0_BASE; //Pointer to oflow IRQ Register
int overflow;
								//(CPU0 -> CPU1)
 ...
 */
int count = 0;
static void handle_cpu5_interrupt(void* context) {
...
static void init_cpu5_isr(void) {
	alt_ic_isr_register(PROCESSOR5_0_CPU_IRQ_0_IRQ_INTERRUPT_CONTROLLER_ID,
			PROCESSOR5_0_CPU_IRQ_0_IRQ, handle_cpu5_interrupt, (void*) NULL,
			(void*) NULL);
}

static void handle_cpu5_oflow_interrupt(void *context) {
	altera_avalon_mutex_lock(mutex, 1);
	{

		overflow = *isr_5_oflow_ptr & 0x100;
		*isr_5_oflow_ptr = 0;
...
static void init_cpu5_oflow_isr(void) {
	alt_ic_isr_register(PROCESSOR5_0_CPU_OFLOW_IRQ_0_IRQ_INTERRUPT_CONTROLLER_ID,
			PROCESSOR5_0_CPU_OFLOW_IRQ_0_IRQ, handle_cpu5_oflow_interrupt, (void*) NULL,
			(void*) NULL);
}
...
int main(void) {
	...
	init_cpu5_isr();										//Initialize the ISR
	init_cpu5_oflow_isr();
