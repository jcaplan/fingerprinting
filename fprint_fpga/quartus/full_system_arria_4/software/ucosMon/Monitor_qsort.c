...
void schedule_task(void* pdata){
	...
	while(1){
		OSFlagPend(schedule_fgrp, 0b1100, OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME, 0, &err);
		...
		OSFlagPend(dma_fgrp, 0xf, OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME, 0, &err);
		int i;
		for(i = 0; i < 1; i ++){
...
void dma_end(void){
	...
	void* rx_buffer = (void*)&answer[0] ; /* pointer to rx buffer */
			...
			rx_buffer = (void*)&answer[1] ; /* pointer to rx buffer */
			if ((rc = alt_dma_txchan_send (txchan[2],
			tx_data,
			size,
			NULL,
			NULL)) < 0){
			...
			if ((rc = alt_dma_rxchan_prepare (rxchan[2],
					rx_buffer,
					size,
					done_dma1,
					NULL)) < 0){
