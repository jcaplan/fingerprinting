...
% 	Model based design and code generation are increasingly important trends in embedded system design. 
	...
	In this thesis, a separate code generation framework is presented which automates the porting of Simulink generated code to systems with two or four Nios II cores with support for software-level error detection using redundant executions. 
% 	Code generation further supports the redundant execution of threads on several cores to detect errors. 
	...
	A framework is presented for the static analysis of binary code to estimate execution times and memory requirements of each task.
	Standard program analysis techniques are borrowed from compiler theory to provide intraprocedural analysis on compiled C code. 
	A novel multicore schedulability analysis is presented for mixed criticality fault tolerant systems along with a design space exploration framework using nested genetic algorithms to determine schedules with better quality of service.
